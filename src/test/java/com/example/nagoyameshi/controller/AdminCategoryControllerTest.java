...
public class AdminCategoryControllerTest {
   ...
   void 管理者でカテゴリ一覧ページを表示できる() throws Exception {
       mockMvc.perform(get("/admin/categories"))
           .andExpect(status().isOk()) // 正常に200 OK
           .andExpect(view().name("admin/categories/index"));
   }

   // 一般ユーザーで403を確認
   @Test
   @WithMockUser(username = "user", roles = {"FREE_MEMBER"})
   void 一般ユーザーは403になる() throws Exception {
       mockMvc.perform(get("/admin/categories"))
